...
//
// Production hardening for full-school (800+) load:
...
//
// CHANGED: vreaction.com (+ www) added to REQUIRED_ORIGINS so scores save
// from the new primary domain. CLIENT_URL default updated to vreaction.com.
...
const CLIENT_URL = process.env.CLIENT_URL || 'https://vreaction.com';
...
// Always allow all Reactific domains regardless of env variable.
// vreaction.com is the primary front door now; the others stay for compatibility.
const REQUIRED_ORIGINS = [
  'https://vreaction.com',
  'https://www.vreaction.com',
...
// Render runs behind a proxy. Lets express-rate-limit see the real client IP.
...
app.post('/api/auth/google', async (req, res) => {
  ...
  try {
    ...
    const publicUser = {
      id: user.id, email: user.email, username: user.username,
      subscription_status: user.subscription_status, role: user.role,
      class_id: user.class_id, avatar_url: picture, needs_class: !user.class_id
    };
...
app.post('/api/classes/join', authRequired, async (req, res) => {
  try {
    ...
    const classResult = await pool.query(`SELECT id, name FROM classes WHERE code = $1`, [code.toUpperCase().trim()]);
    if (!classResult.rows.length) return res.status(404).json({ error: 'Class not found — check your code' });

    const cls = classResult.rows[0];
    await pool.query(
      `INSERT INTO class_students (class_id, user_id) VALUES ($1, $2) ON CONFLICT (class_id, user_id) DO NOTHING`,
      [cls.id, req.user.id]
    );
    await pool.query(`UPDATE users SET class_id = $1, updated_at = NOW() WHERE id = $2`, [cls.id, req.user.id]);
    res.json({ class: cls });
  } catch (err) {
    ...
    const students = await pool.query(
      `SELECT u.id, u.username, u.avatar_url,
       ...
       LEFT JOIN LATERAL (
         ...
         FROM scores WHERE user_id = u.id ORDER BY created_at DESC LIMIT 1
       ) s ON true
       WHERE cs.class_id = $1
  ...
  try {
    const result = await pool.query(
      `SELECT c.id, c.name, c.code, u.username as teacher_name
       FROM users me JOIN classes c ON c.id = me.class_id
       JOIN users u ON u.id = c.teacher_id WHERE me.id = $1`,
      [req.user.id]
    );
...
app.get('/api/classes/teaching', authRequired, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT c.id, c.name, c.code, c.display_mode, c.created_at, COUNT(cs.user_id) as student_count
  ...
  try {
    ...
    const rankResult = await pool.query(
      `SELECT COUNT(*) + 1 AS rank FROM (
         SELECT DISTINCT ON (user_id) user_id, score FROM scores WHERE court = $1 AND speed = $2
    ...
    res.status(201).json({
      id: result.rows[0].id, rank: parseInt(rankResult.rows[0].rank, 10),
      speed: normalizedSpeed, created_at: result.rows[0].created_at
    });
  } catch (err) {
...
async function getLeaderboard(period, speedInput, limitInput) {
  ...
  const result = await pool.query(
    `SELECT DISTINCT ON (s.user_id) u.username, s.score, s.level, s.tier, s.streak, s.created_at
  ...
  const entries = result.rows
    ...
    .slice(0, limit).map((row, i) => ({ rank: i + 1, ...row }));
  ...
  try {
    ...
    const best = await pool.query(
      `SELECT score, level, tier, streak, created_at FROM scores
       WHERE user_id = $1 AND court = 'full' AND speed = $2 ORDER BY score DESC, created_at ASC LIMIT 1`,
      [req.user.id, speed]
    );
    ...
    const rankResult = await pool.query(
      `SELECT COUNT(*) + 1 AS rank FROM (
         ...
         WHERE court = 'full' AND speed = $1 ORDER BY user_id, score DESC, created_at ASC
...
app.post('/api/stripe/checkout', authRequired, async (req, res) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: STRIPE_CANCEL_URL, metadata: { user_id: req.user.id }
    });
...
const VALID_GAMES = ['reaction', 'numhunt', 'recovery', 'pattern', 'sequence', 'focus', 'orbit', 'drill', 'drill100', 'drill120', 'coord80', 'coord100', 'fusion', 'flashspatial', 'flashseq'];
...
app.post('/api/scores/game', authRequired, async (req, res) => {
  try {
    ...
    if (!streakRow.rows.length) {
      await pool.query(`INSERT INTO user_streaks (user_id, current_streak, best_streak, last_played_on) VALUES ($1, 1, 1, $2)`, [req.user.id, today]);
    } else {
      ...
      if (lastPlayed !== today) {
        ...
        await pool.query(`UPDATE user_streaks SET current_streak = $1, best_streak = $2, last_played_on = $3 WHERE user_id = $4`, [newCurrent, newBest, today, req.user.id]);
      }
    }
    ...
    if (ranked) {
      const rankResult = await pool.query(
        `SELECT COUNT(*) + 1 AS rank FROM (
           ...
           WHERE game_id = $1 AND class_id IS NOT NULL ORDER BY user_id, score DESC, created_at ASC
         ) top WHERE top.score > $2`,
    ...
    if (ranked) {
      ...
      const gid = String(game_id).toLowerCase();
      ['today', 'week', 'alltime'].forEach(function (w) { broadcastLeaderboardUpdate(gid, w, null); });
    }

    res.status(201).json({ id: inserted.rows[0].id, created_at: inserted.rows[0].created_at, ranked, rank });
  } catch (err) {
...
async function getGameLeaderboard(gameId, windowParam, classIdParam, limitInput) {
  ...
  const result = await pool.query(
    `SELECT * FROM (
       SELECT DISTINCT ON (gs.user_id)
         ...
         u.username, u.first_name, u.last_name, c.display_mode
       FROM game_scores gs
  ...
  const entries = result.rows.map((row, i) => ({
    rank: i + 1, name: displayName(row), score: row.score, level: row.level
  }));
  ...
  try {
    ...
    const best = await pool.query(
      `SELECT score, level, created_at FROM game_scores
       WHERE user_id = $1 AND game_id = $2 ORDER BY score DESC, created_at ASC LIMIT 1`,
      [req.user.id, gameId]
    );
...
app.get('/api/streak/me', authRequired, async (req, res) => {
  try {
    const result = await pool.query(`SELECT current_streak, best_streak, last_played_on FROM user_streaks WHERE user_id = $1`, [req.user.id]);
...
app.post('/api/auth/student-login', async (req, res) => {
  try {
    ...
    if (!cleanEmail || !cleanCode) return res.status(400).json({ error: 'Email and class code required' });
    if (!/^\S+@\S+\.\S+$/.test(cleanEmail)) return res.status(400).json({ error: 'Valid email required' });
    if (cleanCode.length !== 6) return res.status(400).json({ error: 'Class code must be 6 letters' });

    const classResult = await pool.query(`SELECT id, name FROM classes WHERE code = $1`, [cleanCode]);
    if (!classResult.rows.length) return res.status(404).json({ error: 'Class code not found — check with your teacher' });
    ...
    if (user.class_id !== cls.id) {
      await pool.query(`UPDATE users SET class_id = $1, updated_at = NOW() WHERE id = $2`, [cls.id, user.id]);
      user.class_id = cls.id;
    }

    const publicUser = {
      id: user.id, email: user.email, username: user.username,
      subscription_status: user.subscription_status, role: 'student',
    ...
    if (err.code === '23505') return res.status(409).json({ error: 'Please try again' });
    console.error('Student login error:', err);
    res.status(500).json({ error: 'Server error' });
  }
});

// ── DAILY DRILL — teacher-controlled shared clock ──────────
var activeDrills = {};
var DRILL_DURATION_MS = 5 * 60 * 1000;
...
app.post('/api/drill/start', authRequired, async (req, res) => {
  try {
    ...
    const owns = await pool.query(`SELECT id, name FROM classes WHERE id = $1 AND teacher_id = $2`, [classId, req.user.id]);
    if (!owns.rows.length) return res.status(403).json({ error: 'Not your class' });
    const startAt = Date.now() + 5000;
    activeDrills[classId] = { startAt, durationMs: DRILL_DURATION_MS, startedBy: req.user.id, className: owns.rows[0].name };
...
app.get('/api/drill/status/:class_id', async (req, res) => {
  try {
    ...
    if (now >= endAt) { delete activeDrills[classId]; return res.json({ active: false, ended: true }); }
    res.json({ active: true, startAt: d.startAt, endAt, durationMs: d.durationMs, serverNow: now, className: d.className });
  } catch (err) {
...
wss.on('connection', function (ws) {
  ...
  ws.on('message', function (data) {
    try {
      ...
      if (msg.action === 'subscribe') {
        ...
        if (!VALID_GAMES.includes(gameId)) { ws.send(JSON.stringify({ type: 'error', message: 'Invalid game' })); return; }
        subscriptionKey = gameId + '-' + win + '-' + (classId || 'all');
        if (!activeSubscriptions[subscriptionKey]) activeSubscriptions[subscriptionKey] = [];
  ...
  ws.on('close', function () {
    if (subscriptionKey && activeSubscriptions[subscriptionKey]) {
      ...
      if (idx > -1) activeSubscriptions[subscriptionKey].splice(idx, 1);
      if (activeSubscriptions[subscriptionKey].length === 0) delete activeSubscriptions[subscriptionKey];
    }
  });
...
async function broadcastLeaderboardUpdate(gameId, win, classId) {
  ...
  try {
    ...
    var msg = JSON.stringify({ type: 'leaderboard_update', game_id: gameId, window: win, entries: leaderboard.entries });
    subscribers.forEach(function (ws) { if (ws.readyState === ws.OPEN) ws.send(msg); });
  } catch (err) {
